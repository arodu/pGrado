 ...
 */
	public function getConditions($data = null) {
		...
		foreach ($data as $values) {
			...
			foreach ($values as $key => $value) {
				if($value !== ''){
					$conditions[$concatCondition][] = $this->_buildingCondition($this->_getToken($key,$value));
	...
	private function _buildingCondition($datos){
		$out=null;
		$outF=null;
		foreach ($datos['values']['or'] as $orValues) {
			...
			foreach ($orValues['and'] as $andValues) {
				...
				$aux[] = $this->buildDataValue($andValues);
			}
	...
	private function buildDataValue($datos){
		$out['pre'] = '';
		$out['pos'] = '';

		if($datos['posToken'] === false){
			$out['pre'] =	$this->Controller->modelClass.'.'.$datos['field'].' LIKE';
			$out['pos'] = 	'%'.$datos['value'].'%';
		}else{
			switch ($datos['nombre']) {
				case 'igual': 	$out['pre'] = $this->Controller->modelClass.'.'.$datos['field']; 		break;
				case 'dif': 	$out['pre'] = $this->Controller->modelClass.'.'.$datos['field'].' !='; 	break;
				case 'maigual': $out['pre'] = $this->Controller->modelClass.'.'.$datos['field'].' >=';	break;
				case 'meigual': $out['pre'] = $this->Controller->modelClass.'.'.$datos['field'].' <='; 	break;
				case 'ma': 		$out['pre'] = $this->Controller->modelClass.'.'.$datos['field'].' >'; 	break;
				case 'me': 		$out['pre'] = $this->Controller->modelClass.'.'.$datos['field'].' <'; 	break;

				case 'id': 		$out['pre'] = $this->Controller->modelClass.'.'.$datos['field']; 	break;

				default:		$out['pre'] = $this->Controller->modelClass.'.'.$datos['field'].' LIKE'; break;
			}
	...
	private function _multiText($field, $text, $sep=' '){
		...
		foreach ($porc as $value) {
			...
			//$out[] = $this->_buildingCondition($this->_getToken($field, $text));   // INGRESAR RECURSIVIDAD AQUI ***************************
